...
observeEvent(input$`nav-page`, {
  if (input$`nav-page` == "game" && is.null(input$cookies$consent)) {
    inputSweetAlert(
      ...
      title = "Are you over 18 and in the US?",
      text = paste0(
...
observeEvent(input$risk_state, {
  if (length(geo_county()) > 0) {
    ...
    if (input$risk_state == "USA") {
      ...
      game_counties = usa_counties %>% filter(stname == !!input$risk_state) %>% pull(NAME) %>% sort %>% unique()
    }
  ...
  if (input$cookies$consent == "yes") {
    ...
    query <-
      sqlInterpolate(
        ...
        src = ref_content$utm_source,
        medium = ref_content$utm_medium,
        content = ref_content$utm_content,
        camp = ref_content$utm_campaign
      )
...
observeEvent(input$game_will, {
  sql <-
    "INSERT INTO willingness
      (
        source, asc_bias, event_size,
        answer, ip, latitude, longitude,
