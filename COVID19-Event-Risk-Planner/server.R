...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$uk_event_size_map, {
    output$uk_map_static <- renderUI(
      {
        tags$iframe(src=paste0('uk_', input$uk_asc_bias, "_", input$uk_event_size_map, ".html"),
                    width = 992, height = 500, frameBorder="0")
      }
    )
  })
  ...
  observeEvent(input$italy_event_size_map, {
    output$italy_map_static <- renderUI(
      {
        tags$iframe(src=paste0('italy_', input$italy_asc_bias, "_", input$italy_event_size_map, ".html"),
                    width = 992, height = 500, frameBorder="0")
      }
    )
  })
  ...
  observeEvent(input$swiss_event_size_map, {
    output$swiss_map_static <- renderUI(
      {
        tags$iframe(src=paste0('swiss_', input$swiss_asc_bias, "_", input$swiss_event_size_map, ".html"),
                    width = 992, height = 500, frameBorder="0")
      }
    )
  })
